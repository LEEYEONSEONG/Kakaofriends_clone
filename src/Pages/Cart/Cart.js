    ...
    .then((res) => {
      ...
      this.setState({
        ...
        for(let i = 0; this.state.carts.length > i; i++) {
          isTotalPrice += this.state.carts[i].price * this.state.carts[i].count;
        } this.setState({
      ...
      <a href="https://store.kakaofriends.com/kr/index?tab=hot" className="linkButton">
        ...
        <div className="cartHeaderWrap">
          ...
          <div className="filledCartHeader">      
            <div className="subHeader">
              <label className={`checker ${this.state.isChecked ? "" : "unchecked"}`} >
                <input onClick={this.handleCheck} type="checkbox" className="checkBox" checked={ this.state.isChecked ? "checked" : "unchecked" }></input>
              </label>
        ...
        <div className="filledCartWrap">
          <div className="filledCartDetail">
            ...
            {carts.map(el => {
              return(
                <CartItem carts={el} check={this.handleCheck} />
              )
            })}
          ...
          <span>{delivery ? this.state.isTotalPrice.toLocaleString(): (this.state.isTotalPrice + 3000).toLocaleString() }</span>
                    원</span>
              </div>
            </div>
          </div>
